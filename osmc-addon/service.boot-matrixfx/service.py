...
class RainWindow(xbmcgui.WindowDialog):
    def __init__(self, secs=8.0, columns=48, speed_px=18, font="font13"):
        ...
        # Fullwidth katakana + digits + A–Z (no spaces)
        ...
        # Optional dark backdrop — ControlImage with empty texture works in Kodi Matrix+
        self.bg = xbmcgui.ControlImage(0, 0, self.w, self.h, "")
        self.addControl(self.bg)
        self.bg.setColorDiffuse("FF000000")  # opaque black
    ...
    def _create_columns(self):
        for i in range(self.columns):
            ...
            ctl = xbmcgui.ControlLabel(
                ...
                font=self.font,
                # Make it clearly visible first: bright green head often
                textColor="FF00FF00",
                alignment=0x00000002  # right align for a nice stream look
            )
            self.addControl(ctl)
    ...
    def _tick(self):
        for idx, ctl in enumerate(self.controls):
            ...
            if y > self.h:
                ...
                # occasional flicker
                ctl.setTextColor("FF00FF00" if random.random() > 0.3 else "88CCFF88")
...
class Service(xbmc.Monitor):
    def run(self):
        # Ensure the Home window is visible so we have a GUI surface
        while not xbmc.getCondVisibility("Window.IsVisible(home)"):
            if self.abortRequested():
                return
            xbmc.sleep(150)

        xbmc.sleep(400)  # tiny settle delay
